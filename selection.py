## selection.py
## this script will select the appropriate options using a method derived from VIX near and next term selection methods
import numpy as np
import pandas as pd
import DataPull
import functions
...
def filter_options(df, time_to_maturity=(24, 40)):
    ...
    atm_prices = df.groupby('date')[
        'forward_price'].median().reset_index()  # calculates the median forward price for each date .reset_index() is used to convert series to a dataframe
